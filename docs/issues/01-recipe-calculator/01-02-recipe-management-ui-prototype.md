...
1. **Recipe list** — cards showing recipe name, last cooked date ("Never cooked" if null), ingredient count; sorted most-recently-cooked first; search bar at the top; FAB (Floating Action Button) for "New Recipe" on mobile
...
4. **Recipe detail (read-only)** — ingredient list with template amounts, last cooked date and last cooked weight; "Start Cooking" button (links to Cooking Mode, static in prototype); "Edit" button. No per-100g nutrition — that only exists inside Cooking Mode.
...
- [ ] Recipe list cards show name, last cooked date ("Never cooked" if null), and ingredient count
